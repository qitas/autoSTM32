...
def display_homescreen():
    if not storage.is_initialized():
        label = 'Go to o2fun.com/wallet'
        image = res.load('apps/homescreen/res/oazon.toif')
    else:
        label = storage.get_label() or 'Oazon Wallet'
    ...
    if storage.is_initialized() and storage.unfinished_backup():
        ...
        ui.display.text_center(ui.WIDTH // 2, 22, 'BE BACKUP!!', ui.BOLD, ui.BLACK, ui.YELLOW)
        ui.display.bar(0, 30, ui.WIDTH, ui.HEIGHT - 30, ui.BG)
    else:
